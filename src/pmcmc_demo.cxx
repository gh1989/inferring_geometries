...
int main()
{
    ...
    const double trajectory_diffusion_sigma = 0.0001;
    ...
    C_chain(0,0) = C(0);
    V.set_mode(1,1,C(0));
    marginal_likelihood_c = sequential_monte_carlo( r, x, w, y, phat, V, 
                                                    K, N, T, dt, 
                                                    observation_noise_variance,
                                                    trajectory_diffusion_sigma );
    ...
    for(int i=0; i<trials; ++i)
    {
        ...
        marginal_likelihood_c_star = sequential_monte_carlo( r, x, w, y, phat, V, 
                                                             K, N, T, dt, 
                                                             observation_noise_variance, 
                                                             trajectory_diffusion_sigma );
        sample_smc_approximation( r, K, N, T, x, w, X_star);

        log_acceptance_probability = calculate_log_acceptance_probability( marginal_likelihood_c, 
                                                                           marginal_likelihood_c_star, 
                                                                           C, C_star, 
                                                                           proposal_c_variance);
