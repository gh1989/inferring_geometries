...
int main( int argc, char *argv[] )
{
	...
	// Real constant.
	FourierSeries real_V(1);
	real_V.set_mode( 0, 1, real_parameter_c - 0.5*_Complex_I );
	...
	em_overdamped_langevin( path_steps, path_delta, C, &real_V, real_path, r,start_point);
	...
	chain[0] = c;
	FourierSeries V(1);
	FourierSeries V_star(1);
	V.set_mode( 0, 1, c - 0.5*_Complex_I );
	...
	// With V the Fourier series containing modes c, generate a path and store in x.
	em_overdamped_langevin( path_steps, path_delta, C, &V, x, r,start_point);
	
	for( int i=0; i<mcmc_trials+burn; ++i )
	{
		c_star = gsl_ran_gaussian( r, param_delta ) + c; // c* ~ N(c, param_delta).
		V_star.set_mode( 0, 1, c_star - 0.5*_Complex_I );
		V_star.set_mode( 1, 0, 0.5 - 0.5*_Complex_I );
		em_overdamped_langevin( path_steps, path_delta, C, &V_star, x_star, r, start_point); // Generate x*
		
		u = gsl_rng_uniform(r);
		log_alpha = log_p( c_star, param_delta) - log_p( c, param_delta);
		//log_alpha += log_g( c, c_star, param_delta) - log_g( c_star, c, param_delta); // Since this is symmetric it is always zero.
		...
		if (u<alpha)
		{
			...
			// Set c to c*, and set_mode 1, 0 to c* on the potential.
			c = c_star;
			V.set_mode( 0, 1, c_star - 0.5*_Complex_I );
	...
	printf("Average value for constant from chain: %f. \n", average_constant);
	...
	// Moving average data.
	FILE *fp;
	fp = fopen("Testing/MCMC/single_param_data.txt", "w+");
	fprintf(fp, "#n\tmean\n");
	...
	// Time series data.
	fp = fopen("Testing/MCMC/single_param_data_ts.txt", "w+");
	fprintf(fp, "#n\tvalue\n");
